...
def render_zsh(parser: argparse.ArgumentParser, prog: str = "talaria") -> str:
    ...
    return f"""#compdef _talaria {prog}
# zsh completion for {prog} — generated by `{prog} completion zsh`.
# The `#compdef _funcname <cmd>` form (vs the bare `#compdef <cmd>`
# autoload form) registers the function as the completion for `{prog}`
# when the script is sourced OR autoloaded — so `eval "$(talaria
# completion zsh)"` in ~/.zshrc wires up completion without needing
# the file in $fpath. There is intentionally NO trailing `_talaria
# "$@"` call: that would invoke `_arguments` in the wrong context
# when the script is sourced (raising `_arguments:comparguments:…
